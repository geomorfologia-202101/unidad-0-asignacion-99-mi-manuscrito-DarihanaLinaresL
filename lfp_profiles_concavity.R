LfpProfilesConcavity <- function(xycoords, network, prefix, dem, direction,
  ...
  #   ...:          Other arguments passed to stats::spline function.
  ...
  writeVECT(
    sp,
    paste0('LfpNetwork_outlet_', prefix),
  ...
  execGRASS(
    ...
    flags='overwrite',
    parameters = list(
      input = paste0('LfpNetwork_outlet_', prefix),
      output = paste0('LfpNetwork-outlet-', prefix),
      use = 'cat'
    )
  )
  ...
  execGRASS(
    ...
    flags='overwrite',
    parameters = list(
      stream_rast = paste0('LfpNetwork-outlet-', prefix),
      direction = direction,
      distance = paste0('LfpNetwork-flds-', prefix),
      method = 'downstream'
    )
  )
  ...
  length <- raster(readRAST(paste0('LfpNetwork-flds-', prefix)))
  names(length) <- 'length'
    ...
    function(x)
      as.data.frame(
        tryCatch(
          ...
          error=function(e){
            foo <- dfs[dfs$stream==x,c('length','z')]
            colnames(foo) <- c('x','y')
            return(foo)})
      ),
    simplify = F
