...
Para el estudio morfométrico de la cuenca Guayubín se usó softwares de código abierto como medio para procesar datos estadísticos y modelos digitales con la finalidad de generar las informaciones ha analizada.
...
## Materiales
RStudio. Donde se redactó el manuscrito, se procesaron los datos que ofrece el DEM de la cuenca a traves de un script en el que se usaron paquetes que produjeran los resultados. Se usaron las siguientes colecciones de funciones y conjunto de datos en el script: 
...
## Metodología
Los parametros de la cuenca fueron calculados por medio de el addon `r.watershed` (@), utilizando el Dem de la misma, los parametros calculados fueron acumulación, elevación, depresión, drenaje, flujo, cuenca y media cuenca, con un umbral de acumulación de flujo de 80 celdas necesarias para que exista una red de agua.

Se usó el addon `r.water.outlet` (@) para la extraccion de la cuenca a traves de un mapa de direccion de drenajes (creado con el addon r.watershed), y de coordenadas de desembocaduras de la cuenca (obtenidas con el paquete `Mapview`). Asi tambien se uso el addon `r.to.vect` (@), para convertir un raster en vectorial.

Para estraer la red de drenaje se hizo uso de el addon `r.stream.extract` (@), donde se utilizo un raster como capa de entrada y se genero un archivo en formato vectorial, ademas, del raster, es necesario el umbral de acumulacion de flujo.

En cuanto al orden de red y el analisis hortoniano, se uso el addon `r.stream` (@). Se creo un mapa de direccion de flujo con `r.stream.extract` (@), que sirvio en la creacion de mapas de ordenes de redes generados con `r.stream.order` (@). ademas de el mapa de direccion de flujo, se uso un raster de red de la cuenca, flujo de acumulacion y un modelo de elevacion. Se calcularon los ordenes de redes segun las clasificaciones de Strahler, Horton, Shreve, Hack y Topo, pero, se uso la red de orden segun Strahler para delimitar la cuenca. Mientras que se usaron los addons `r.info` para obtener los valores minimos y maximos del orden de red segun Strahler; para delimitar la cuenca a traves de la red de drenaje se utilizo `r.stream.basins` (@); y para calcular las estadisticas de red de Horton ordenadas para redes de orden de Strahler y Horton se uso `r.stream.stats` (@).

Para calcular los indices de concavidad y los perfiles longitudinales, primero, se obtuvieron los cursos mas largos de la cuenca a traves de la funcion `LfpNetwork` (@), usando coordenadas de desembocadura de la cuenca obtenidas con `Mapview`, vectores de ordenes de red y mapa de flujo de direccion. Segundo, para producir los perfiles longitudinales e indices de cocavidad se empleo la funcion `LfpProfilesConcavity` (@), utilizando la red de cursos de agua mas largos, coordenadas de desembocadura, un dem y un mapa de flujos de drenaje.

Para la obtencion de los parametros morfometricos de la cuenca se uso el addon `r.basin` (@), por medio de un modelo de elevacion y coordenadas proyectadas de la salida de la cuenca, obtenidas en geograficas con `Mapview` y transformadas a proyectadas con la funcion `My_Trans` (@). 

Respecto al calculo de la curva e integral hipsométrica, se utilizo la funcion `HypsoIntCurve` (@),
